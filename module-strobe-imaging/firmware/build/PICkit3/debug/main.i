...
uint32_t find_scalers_time( uint32_t target_time_ns, uint8_t *prescale, uint8_t *postscale, uint8_t *period )
{
    ...
    if ( target_time_ns > ( ( ( (uint32_t)( 1000000000 / ( 32000000 / 1000 ) ) << 7 ) / 1000 ) * 16 * 255 ) )
        return 0;
    ...
    ticks = ( target_time_ns * ( 1000 / 10 ) + ( ( ( 1000000000 / ( 32000000 / 1000 ) ) >> 1 ) / 10 ) ) / ( ( 1000000000 / ( 32000000 / 1000 ) ) / 10 );
    ...
    for ( postscale_loop=16; postscale_loop>=1; postscale_loop-- )
    {
        ...
        for ( prescale_loop=7; prescale_loop>=0; prescale_loop-- )
        {
            if ( prescale_loop == 0 )
                ...
                period_loop = ( ( rem >> ( prescale_loop - 1 ) ) + 1 ) >> 1;

            if ( ( period_loop > 0 ) && ( period_loop <= 0xFF ) && ( ( period_loop > 1 ) || ( prescale_loop > 0 ) ) )
            {
                time_ns_loop = ( ( ( ( (uint32_t)( 1000000000 / ( 32000000 / 1000 ) ) / 10 ) << prescale_loop ) * period_loop ) * postscale_loop ) / ( 1000 / 10 );
...
# 192 "main.c"
    while ( 1 )
    {
        if ( spi_packet_read( &spi_packet, &packet_type, (uint8_t *)&packet_data, &packet_data_size, 32 ) == 0 )
        {
            switch ( packet_type )
            {
                case 0:
