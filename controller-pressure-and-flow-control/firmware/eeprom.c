...
extern ee_status_t eeprom_read_status( void )
{
    ...
    buf[1] = 0xFF;
    SPI3_EE_SS_SetLow();
    SPI3_Exchange8bitBuffer( buf, 2, buf );
    SPI3_EE_SS_SetHigh();
...
extern uint8_t eeprom_read_byte( uint16_t addr )
{
    ...
    SPI3_EE_SS_SetLow();
    SPI3_Exchange8bitBuffer( buf, READ_INSTR_BYTES+1, buf );
    SPI3_EE_SS_SetHigh();
...
#endif
    SPI3_EE_SS_SetLow();
    SPI3_Exchange8bitBuffer( buf, READ_INSTR_BYTES, NULL );
    SPI3_Exchange8bitBuffer( NULL, num, data );
    SPI3_EE_SS_SetHigh();
}
...
extern uint8_t eeprom_verify_bytes( uint16_t addr, uint8_t num, uint8_t *data )
{
    ...
    while ( num )
    {
        SPI3_Exchange8bitBuffer( NULL, 1, buf );
...
extern void eeprom_set_wren( bool wren )
{
    ...
    SPI3_EE_SS_SetLow();
    SPI3_Exchange8bitBuffer( buf, 1, NULL );
    SPI3_EE_SS_SetHigh();
}
...
extern void eeprom_write_byte( uint16_t addr, uint8_t byte )
{
    ...
    SPI3_EE_SS_SetLow();
    SPI3_Exchange8bitBuffer( buf, WRITE_INSTR_BYTES+1, NULL );
    SPI3_EE_SS_SetHigh();
...
//        printf( "Write %hu to %hu, %hu bytes\n", addr, addr+count-1, count );
    ...
    return;
        ...
        SPI3_EE_SS_SetLow();
        SPI3_Exchange8bitBuffer( buf, WRITE_INSTR_BYTES, NULL );
        SPI3_Exchange8bitBuffer( data, count, NULL );
        SPI3_EE_SS_SetHigh();
